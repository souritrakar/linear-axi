...
5. Apply `references/approval-policy.md`. When step 3 produced no ambiguity, every folded answer is
   verified, and the rewrite changes no scope, date, or product decision, proceed without asking and
   report the before/after and the evidence in your result. Otherwise present the complete
   before/after, verification evidence, auto-folded answers, ambiguities, and repository choice, then
   wait for explicit approval and ambiguity resolution.
...
When a blocker is what changed, do not search for a blocked state. Record it with
`linear relate <id> --to <blocker-id> --type blocks` plus a comment naming what would unblock it.
